...
flags.DEFINE_enum('processor_type', 'edge_t',
                  ['deepsets', 'mpnn', 'pgn', 'pgn_mask',
                   ...
                   'triplet_gpgn', 'triplet_gpgn_mask', 'triplet_gmpnn', 'edge_t', 'branching_edge_t', "branching_gatv2"],
...
flags.DEFINE_list('remap_algo_from_list', None,
                      'List of algorithm names to remap from when loading pretrained processor weights.')
flags.DEFINE_list('remap_algo_to_list', None,
                      'List of algorithm names to remap to when loading pretrained processor weights.')
...
flags.DEFINE_integer('subset_id', 0,
                      'Subset id, used for multi-task training on subsets of CLRS.')
flags.DEFINE_string('save_name', 'default', 'Name for saving results.')
  ...
  for run in range(FLAGS.runs):
    ...
    while step < FLAGS.train_steps:
      ...
      if step == 0:
        ...
        if os.path.exists(load_checkpoint_path):
          ...
          # load the flattened pretrained parameter updates and apply them
          remap_algo_from_list = [int(x) for x in FLAGS.remap_algo_from_list] if FLAGS.remap_algo_from_list else None
          remap_algo_to_list = [int(x) for x in FLAGS.remap_algo_to_list] if FLAGS.remap_algo_to_list else None
          utils.assign_to_model_parameters(train_model, remap_algo_from_list=remap_algo_from_list, remap_algo_to_list=remap_algo_to_list, model_type=FLAGS.processor_type)
    ...
    for algo_idx in range(len(train_samplers)):
      ...
      for k, v in val_stats.items():
        key = f'val_algo_{FLAGS.algorithms[algo_idx]}_{k}'
        if key in metrics:
          metrics[key].append(v)
        else:
          metrics[f'val_algo_{FLAGS.algorithms[algo_idx]}_{k}'] = [v]
      for k, v in test_stats.items():
        key = f'test_algo_{FLAGS.algorithms[algo_idx]}_{k}'
        if key in metrics:
          metrics[key].append(v)
        else:
          metrics[f'test_algo_{FLAGS.algorithms[algo_idx]}_{k}'] = [v]
    ...
    for split in ['val', 'test']:
      for key, value in metrics.items():
        if key.startswith(f"{split}_algo_{algo}_"):
          ...
          rows.append({
            'subset_id': FLAGS.subset_id,
            'target_algorithm': algo,
            'training_algorithms': ','.join(FLAGS.algorithms),
            'split': split,
            'metric': metric,
            'value': np.mean(value),
            'std': np.std(value)
