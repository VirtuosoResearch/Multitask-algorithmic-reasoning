...
class GATLayer(hk.Module):
  """Single Graph Attention Network layer (Velickovic et al., ICLR 2018)."""
  ...
  def __call__(
    ...
    """Single GAT layer inference step."""
...
class GAT(Processor):
  """Multi-layer Graph Attention Network (Velickovic et al., ICLR 2018)."""
  ...
  def __call__(  # pytype: disable=signature-mismatch  # numpy-scalars
    ...
    """Multi-layer GAT inference step."""
...
class GATFull(Processor):
  """Multi-layer Graph Attention Network with full adjacency matrix."""
  ...
  def __call__(self, node_fts: _Array, edge_fts: _Array, graph_fts: _Array,
    ...
    return self.gat(node_fts, edge_fts, graph_fts, adj_mat, hidden, **unused_kwargs)


class GATv2Layer(hk.Module):
  """Single Graph Attention Network v2 layer (Brody et al., ICLR 2022)."""
  ...
  def __call__(
    ...
    """Single GATv2 layer inference step."""
...
class GATv2(Processor):
  """Multi-layer Graph Attention Network v2 (Brody et al., ICLR 2022)."""
  ...
  def __call__(  # pytype: disable=signature-mismatch  # numpy-scalars
    ...
    """Multi-layer GATv2 inference step."""
    
    # Apply GATv2 layers sequentially
    nxt_hidden = hidden
    for _ in range(self.num_layers):
      nxt_hidden = GATv2Layer(
          out_size=self.out_size,
          nb_heads=self.nb_heads,
...
class BranchingGATv2(hk.Module):
  """Branching Graph Attention Network v2."""
  ...
  def __init__(
    ...
    super().__init__(name="BranchingGATv2")
    self.out_size = out_size
    self.nb_heads = nb_heads
    self.num_layers = num_layers
    self.mid_size = mid_size
  ...
  def __call__(
    ...
    """BranchingGATv2 inference step."""
    # First create the modules
    gatv2_layers = []
    for i in range(self.num_layers):
      cur_layer = []
      for _ in range(self.num_modules[i]):
        cur_layer.append(GATv2Layer(
            out_size=self.out_size,
            nb_heads=self.nb_heads,
            mid_size=self.mid_size,
            activation=self.activation,
      ...
      gatv2_layers.append(cur_layer)

    # Apply layers according to branching structure
    nxt_hidden = hidden
    for i in range(self.num_layers):
      module_idx = self.branching_structure[i][algorithm_idx]
      nxt_hidden = gatv2_layers[i][module_idx](
          node_fts, edge_fts, graph_fts, adj_mat, nxt_hidden
      )
...
class Branching_GATv2_Processor(Processor):
  """Branching Graph Attention Network v2 Processor."""
  ...
  def __call__(  # pytype: disable=signature-mismatch  # numpy-scalars
    ...
    """Branching GATv2 inference step."""
    
    transformer = BranchingGATv2(
        out_size=self.out_size,
        nb_heads=self.nb_heads,
        num_layers=self.num_layers,
        mid_size=self.mid_size,
    ...
    algorithm_idx = kwargs.get('algorithm_index', 0)
    nxt_hidden = transformer(
        node_fts, edge_fts, graph_fts, adj_mat, hidden, algorithm_idx
    )
    
    return nxt_hidden, None  # pytype: disable=bad-return-type  # numpy-scalars


class GATv2Full(Processor):
  """Multi-layer Graph Attention Network v2 with full adjacency matrix."""
  ...
  def __call__(self, node_fts: _Array, edge_fts: _Array, graph_fts: _Array,
               adj_mat: _Array, hidden: _Array, **unused_kwargs) -> _Array:
    adj_mat = jnp.ones_like(adj_mat)
    return self.gatv2(node_fts, edge_fts, graph_fts, adj_mat, hidden, **unused_kwargs)
...
class PGNLayer(hk.Module):
  """Single Pointer Graph Networks layer (Veličković et al., NeurIPS 2020)."""
  ...
  def __call__(
    ...
    """Single PGN layer inference step."""
...
class PGN(Processor):
  """Multi-layer Pointer Graph Networks (Veličković et al., NeurIPS 2020)."""
  ...
  def __call__(  # pytype: disable=signature-mismatch  # numpy-scalars
    ...
    """Multi-layer PGN inference step."""
    
    # Apply PGN layers sequentially
    nxt_hidden = hidden
    tri_msgs = None
    for _ in range(self.num_layers):
      nxt_hidden, tri_msgs = PGNLayer(
          out_size=self.out_size,
          mid_size=self.mid_size,
    ...
    return nxt_hidden, tri_msgs  # pytype: disable=bad-return-type  # numpy-scalars


class BranchingPGN(hk.Module):
  """Branching Pointer Graph Networks."""
  ...
  def __init__(
    ...
    super().__init__(name="BranchingPGN")
    self.out_size = out_size
    self.num_layers = num_layers
    self.mid_size = mid_size
    self.mid_act = mid_act
  ...
  def __call__(
    ...
    """BranchingPGN inference step."""
    # First create the modules
    pgn_layers = []
    for i in range(self.num_layers):
      cur_layer = []
      for _ in range(self.num_modules[i]):
        cur_layer.append(PGNLayer(
            out_size=self.out_size,
            mid_size=self.mid_size,
            mid_act=self.mid_act,
            activation=self.activation,
    ...
    for i in range(self.num_layers):
      module_idx = self.branching_structure[i][algorithm_idx]
      nxt_hidden, tri_msgs = pgn_layers[i][module_idx](
          node_fts, edge_fts, graph_fts, adj_mat, nxt_hidden
      )
    
    return nxt_hidden, tri_msgs


class Branching_MPNN_Processor(Processor):
  """Branching Message-Passing Neural Network Processor."""
  ...
  def __call__(  # pytype: disable=signature-mismatch  # numpy-scalars
    ...
    """Branching MPNN inference step."""
    
    # MPNN uses full adjacency matrix
    adj_mat = jnp.ones_like(adj_mat)
    
    branching_pgn = BranchingPGN(
        out_size=self.out_size,
        num_layers=self.num_layers,
        mid_size=self.mid_size,
        mid_act=self.mid_act,
    ...
    algorithm_idx = kwargs.get('algorithm_index', 0)
    nxt_hidden, tri_msgs = branching_pgn(
        node_fts, edge_fts, graph_fts, adj_mat, hidden, algorithm_idx
    )
...
class DeepSets(Processor):
  ...
  def __call__(self, node_fts: _Array, edge_fts: _Array, graph_fts: _Array,
    ...
    return self.pgn(node_fts, edge_fts, graph_fts, adj_mat, hidden, **unused_kwargs)


class MPNN(Processor):
  """Multi-layer Message-Passing Neural Network (Gilmer et al., ICML 2017)."""
...
def get_processor_factory(kind: str,
  ...
  def _factory(
    ...
    elif kind == 'branching_gatv2':
      processor = Branching_GATv2_Processor(
          out_size=out_size,
          nb_heads=nb_heads,
          num_layers=num_layers,
          use_ln=use_ln,
