...
class BaselineModel(model.Model):
  ...
  def restore_model(self, file_name: str, only_load_processor: bool = False):
    ...
    with open(path, 'rb') as f:
      ...
      # Reinitialize optimizer state to match the (possibly remapped) params.
      # Restoring checkpoint optimizer state directly can fail when the
      # checkpoint and current model have different parameter keys.
      try:
        self.opt_state = self.opt.init(self.params)
        logging.info('Restored params merged; optimizer state reinitialized.')
...
def filter_layers(key, layer_threshold, model_type = "mpnn"):
    ...
    if ('processor' in key) and (key_word in key) and "layer_norm" not in key:
        ...
        if key[(key_word_index + len(key_word))%len(key)] == "_":
            # if model_type == "gatv2":
            #   cur_layer = int(key[key_word_index + len(key_word)+1:])//6
            # else:
