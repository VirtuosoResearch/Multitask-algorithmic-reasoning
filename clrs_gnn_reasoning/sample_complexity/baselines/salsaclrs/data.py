...
from .cache_sampler import build_sampler, SAMPLERS
...
class SALSACLRSDataset(Dataset):
    def __init__(self, root, 
                 ...
                 hints=True, ignore_all_hints=False, nickname=None, 
                 graph_generator="er", 
                 graph_generator_kwargs={"n": 16, "p": 0.1},
                 max_cores=-1, 
                 use_graph_batch=True,
                 graph_batch_dir='./graph_batches',
                 **kwargs):
        ...
        Args:
            ...
            forbidden_duplicates_datasets (list): List of datasets for which duplicates are forbidden.
            ...
            use_graph_batch (bool): Whether to use pre-generated graph batches.
            graph_batch_dir (str): Directory containing pre-generated graph batches.
        ...
        self.batch_graph_generation = True
        self.use_graph_batch = use_graph_batch
        ...
        self.graph_batch_path = generate_graph_batch(
            algorithm=algorithm,
            ...
            seed=self.seed,
            batch_dir=self.graph_batch_dir
        )
        ...
        print(root, algorithm, split, name)
        root = osp.join(root, algorithm, split, name)
        if not os.path.exists(root):
            os.makedirs(root, exist_ok=True)
        
        # Initialize graph batch directory
        # if self.batch_graph_generation:
        #     if self.graph_batch_dir is None:
        #         self.graph_batch_dir = osp.join(osp.dirname(root), 'graph_batches')
        #     self._init_graph_batch()
    ...
    def _get_sample(self, idx):
        """Get sample, using pre-generated graph if batch generation is enabled."""
        if self.use_graph_batch:
            # Load pre-generated graph
            graph_data = load_graph_from_batch(self.graph_batch_path, idx)
            
            # Run algorithm on the graph
            algorithm = get_algorithm(self.algorithm)
            _, probes = algorithm(*graph_data)
            inp, outp, hints = probing.split_stages(probes, self.specs)
            
            return to_sparse_data(inp, hints, outp, not self.ignore_all_hints)
        else:
            # Original behavior - generate graph on the fly
            inp, outp, hints = self.sampler.next()
            return to_sparse_data(inp, hints, outp, not self.ignore_all_hints)

    def _get_sample_with_batch_graph(self, graph_idx):
        """Get sample using pre-generated graph from batch."""
        # Load pre-generated graph
        graph_file = self.graph_batch_path / f"graph_{graph_idx}.pkl"
    ...
    def process(self):
        ...
        if self.use_graph_batch:
            logger.info(f"Using pre-generated graphs from {self.graph_batch_path}")
        ...
        if self.max_cores != -1:
            # We don't compute the full batch at once to save on memory (mainly relevant for hints and long algo iterations)
            with mp.Pool(processes=self.max_cores) as pool: 
                while i < self.num_samples:
                    batch_indices = list(range(i, min(i + 500, self.num_samples)))
                    
                    for data in pool.imap_unordered(self._get_sample, batch_indices, chunksize=1):
                        ...
                        while (True):
                            ...
                            if self.verify_duplicates:
                                if self._is_duplicate(data, _dataset[data.edge_index.shape]):
                                    # Get a new sample with different index
                                    replacement_idx = (i + idx + self.num_samples) % self.num_samples
                                    generated_data[idx] = self._get_sample(replacement_idx)
                                    continue
                                else:
                    ...
                    while (True):
                        ...
                        if self.verify_duplicates:
                            if self._is_duplicate(data, _dataset[data.edge_index.shape]):
                                replacement_idx = (i + idx + self.num_samples) % self.num_samples
                                if self.batch_graph_generation:
                                    generated_data[idx] = self._get_sample_with_batch_graph(replacement_idx)
                                else:
                                    inp, outp, hints = self.sampler.next()
...
class SALSACLRSDataset_(Dataset):
    def __init__(self, root, 
                 split,
                 algorithm,
                 num_samples = 1000,
...
def generate_graph_batch(algorithm, graph_generator, graph_generator_kwargs, 
                        split, num_samples, seed, batch_dir='./graph_batches'):
    """Pre-generate a batch of graphs for dataset creation.
    
    Args:
        algorithm: Algorithm name (used to get the sampler)
        graph_generator: Graph generator type ('er', 'ws', etc.)
        graph_generator_kwargs: Parameters for graph generation
        split: Dataset split ('train', 'val', 'test')
        num_samples: Number of graphs to generate
        seed: Random seed
        batch_dir: Directory to store graph batches
    
    Returns:
        Path to the batch directory containing the graphs
    """
    # Create batch directory structure
    batch_root = Path(batch_dir)
    
    # Create unique batch identifier
    batch_name = f"{graph_generator}_"
    
    # Handle different parameter types (ranges vs single values)
    for k, v in sorted(graph_generator_kwargs.items()):
        if isinstance(v, (list, tuple)):
            batch_name += f'{k}{v[0]}-{v[-1]}_'
        else:
            batch_name += f'{k}{v}_'
    
    batch_name += f"seed{seed}"
    
    # Organize by split
    batch_path = batch_root / split / batch_name
    batch_path.mkdir(parents=True, exist_ok=True)
    
    # Check if batch already exists
    info_file = batch_path / "info.pkl"
    if info_file.exists():
        with open(info_file, 'rb') as f:
            info = pickle.load(f)
        if info['num_samples'] >= num_samples:
            logger.info(f"Using existing graph batch at {batch_path} with {info['num_samples']} samples")
            return batch_path
    
    # Generate new batch
    logger.info(f"Generating {num_samples} graphs for {split} split at {batch_path}")
    
    # Create sampler for graph generation
    ...
    # Generate graphs
    for i in trange(num_samples, desc=f"Generating {split} graphs"):
        # Set different seed for each graph to ensure variety
        sampler._rng = np.random.RandomState(seed + i)
        
        # Generate graph input data (just the graph, not algorithm output)
        graph_data = sampler._sample_data()
        
        # Save graph data
        graph_file = batch_path / f"graph_{i}.pkl"
    ...
    logger.info(f"Generated {num_samples} graphs at {batch_path}")
    return batch_path


def load_graph_from_batch(batch_path, index):
    """Load a specific graph from a batch.
    
    Args:
        batch_path: Path to the batch directory
        index: Index of the graph to load
    
    Returns:
        Graph data (list of arrays as expected by algorithms)
    """
    graph_file = Path(batch_path) / f"graph_{index}.pkl"
    with open(graph_file, 'rb') as f:
