...
def train(model, datamodule, cfg, enable_wandb, specs, seed=42, checkpoint_dir=None, node=None, size=None, algorithm=None):
    if enable_wandb:
        wandblogger = pl.loggers.WandbLogger(project=cfg.LOGGING.WANDB.PROJECT, entity=cfg.LOGGING.WANDB.ENTITY, group=cfg.LOGGING.WANDB.GROUP, name=cfg.RUN_NAME+f"-{algorithm}-{node}-{size}")
    else:
        wandblogger = None
...
if __name__ == '__main__':
    ...
    parser.add_argument("--node", type=int, default=4)
    parser.add_argument("--size", type=int, default=1000)
    parser.add_argument("--algorithm", type=str, default="bfs", help="Algorithm to use (e.g., bfs, dfs, etc.)")
    ...
    train(model, datamodule, cfg, args.enable_wandb, train_ds.specs, seed = args.seed, checkpoint_dir=ckpt_dir, node=args.node, size=args.size, algorithm=args.algorithm)