...
class BranchedMTL_EncodeProcessDecode(torch.nn.Module):
    def __init__(self, task_to_specs, cfg):
        ...
        # Create a branched main body of the network
        self.processor = torch.nn.ModuleList()
        for _ in range(self.cfg.MODEL.MSG_PASSING_STEPS):
            layer_processor = torch.nn.ModuleList([Processor(cfg, self.has_randomness)])
            self.processor.append(layer_processor)
        # Create the mapping from tasks to processor modules for each step
        self.task_to_processor_indexes = []
        for _ in range(self.cfg.MODEL.MSG_PASSING_STEPS):
            self.task_to_processor_indexes.append({task: 0 for task in self.tasks}) # intialize mapping all tasks to the first module in each layer
        ...
        if not self.processor[0][0].has_edge_weight() and not self.processor[0][0].has_edge_attr():
            if "A" in specs:
        ...
        elif self.processor[0][0].has_edge_weight():
            self.edge_weight_name = "edge_weight"
        elif self.processor[0][0].has_edge_attr():
    ...
    def branch_layers(self, layer_index, task_subset):
        ''' Create a new branch for the given tasks at a layer'''
        # split at layer index also at following layers
        for i in range(layer_index, len(self.processor)):
            self.processor[i].append(Processor(self.cfg, self.has_randomness))
            # make new split for task_subset 
            self.task_to_processor_indexes[i] = {task: len(self.processor[i])-1 \
                                                        if task in task_subset else \
                                                        self.task_to_processor_indexes[i][task] for task in self.tasks}
    ...
    def forward(self, batch):
        ...
        for step in range(max_len): 
            ...
            for message_passing_step in range(self.cfg.MODEL.MSG_PASSING_STEPS): 
                layer_processor_index = self.task_to_processor_indexes[message_passing_step][task_name]
                ''' Process: one GNN layer; hidden is updated very message passing step '''
                hidden = self.processor[message_passing_step][layer_processor_index](input_hidden, hidden, last_hidden, randomness=randomness[:, step] if randomness is not None else None, edge_index=batch.edge_index, batch_assignment=batch.batch, 
