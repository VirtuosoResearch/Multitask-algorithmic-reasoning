...
output_accuracy = np.array([0, 70.03, 99.73, 99.99, 100.00, 100.00, 100.00, 100.00])

subtraction = np.array([0, 49.56, 94.13, 99.99, 99.99, 99.99, 99.99, 99.99])
x = np.arange(len(output_accuracy))
ax2.scatter(x, output_accuracy, s=150, color="royalblue", marker="o")
ax2.scatter(x, subtraction, s=150, color="forestgreen", marker="o")
p2 = ax2.plot(x, output_accuracy, lw = 4, color="royalblue", linestyle="dashed", label=r"$\mathrm{Output~only}$")
p1 = ax2.plot(x, subtraction, lw = 4, color="forestgreen", linestyle="solid", label=r"$\mathrm{With~intermediate}$")
...
ax2.set_title(r'$\mathrm{Subtraction~}(m=5)$', fontsize = 32)
ax2.tick_params(labelsize=32)
ax2.grid(ls=':', lw=0.8)
plt.legend(fontsize=28, loc="lower right")

plt.tight_layout()
plt.savefig("./figures/sample_complexity_subtraction.pdf", format="pdf", dpi=1200)
plt.show()
