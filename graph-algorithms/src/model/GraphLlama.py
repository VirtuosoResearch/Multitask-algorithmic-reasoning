...
class GraphLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        if graph_tower is not None and (input_ids.shape[1] != 1 or self.training) and graph_data is not None:
            ...
            with torch.no_grad():
                # Encoding the graph data with no gradients (not training the graph encoder)
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                if (cur_input_ids == graph_tower.config.graph_patch_token).sum() == 0: # no patch token in the sequence
                ...
                if graph_tower.config.use_graph_start_end:
                    ...
                    for graph_start_token_pos in graph_start_tokens:
                        ...
                        num_patches = cur_graph_features.shape[0]
                        # the length from start to end should match the number of patches
                        ...
                        if orig_embeds_params is not None:
                            cur_new_input_embeds = torch.cat(
                                (cur_input_embeds[:graph_start_token_pos].detach(), # the difference is using detach here, not training the original embeddings
                                 ...
                                 cur_input_embeds[graph_start_token_pos + num_patches + 2:].detach()), dim=0)
                        else:
                            cur_new_input_embeds = torch.cat(
                                (cur_input_embeds[:graph_start_token_pos+1], 
                                 cur_graph_features, 
                                 cur_input_embeds[graph_start_token_pos + num_patches + 1:]), dim=0)
                        cur_graph_idx += 1
...
class GraphLlamaForCausalLM(LlamaForCausalLM):
    ...
    def initialize_graph_tokenizer(self, use_graph_start_end, tokenizer, device,
        ...
        tokenizer.add_tokens([DEFAULT_GRAPH_PATCH_TOKEN], special_tokens=True) # Add a special token
        self.resize_token_embeddings(len(tokenizer))

        if use_graph_start_end:
            num_new_tokens = tokenizer.add_tokens([DEFAULT_G_START_TOKEN, DEFAULT_G_END_TOKEN], special_tokens=True) # Add special tokens, in total 3 new tokens
            self.resize_token_embeddings(len(tokenizer))
            ...
            if num_new_tokens > 0:
                # get the word embeddings; initialize the new tokens with the average of the embeddings of existing tokens
            ...
            if tune_graph_mlp_adapter:
                ...
                for p in self.get_input_embeddings().parameters():
                    p.requires_grad = True # make the embeddings trainable
                for p in self.get_output_embeddings().parameters():
                    p.requires_grad = False # make the output embeddings (lm head) not trainable
