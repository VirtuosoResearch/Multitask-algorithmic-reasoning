...
import clrs_text_tasks.name_dictionaries as name_dictionaries
...
def adjacency_encoder(graph, name_dict, add_weights=False):
...
def friendship_encoder(graph, name_dict, add_weights=False):
...
def coauthorship_encoder(graph, name_dict, add_weights=False):
...
def incident_encoder(graph, name_dict, add_weights=False):
  ...
  for source_node in graph.nodes():
    ...
    target_nodes_str = ""; edge_weights_str = ""
    nedges = 0
    ...
    if add_weights:
      if nedges > 1:
        output += "Node %s is connected to nodes %s with weights %s.\n" % (
            source_node,
            target_nodes_str[:-2],
            edge_weights_str[:-2]
        )
      elif nedges == 1:
        output += "Node %d is connected to node %s with weights %s.\n" % (
            source_node,
            target_nodes_str[:-2],
            edge_weights_str[:-2]
        )
...
def social_network_encoder(graph, name_dict, add_weights=False):
...
def expert_encoder(graph, name_dict, add_weights=False):
  nodes_string = create_node_string(name_dict, len(graph.nodes()))
  output = (
...
def encode_graph(graph, text_encoder, add_weights=False):
  """Encoding a graph according to the given text_encoder method."""
  name_dict = TEXT_ENCODER_DICT[text_encoder]
  return TEXT_ENCODER_FN[text_encoder](graph, name_dict, add_weights=add_weights)
