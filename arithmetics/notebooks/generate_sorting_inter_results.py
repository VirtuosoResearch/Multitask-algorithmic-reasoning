    ...
    for _ in range(length):
        ...
        for i in range(1, length):
            ...
            if array[i - 1] > array[i]:
                ...
                intermediate_variables.append([deepcopy(i-1), deepcopy(i)])
...
def insertion_sort(array):
    ...
    for j in range(1, len(array)):
        ...
        while i >= 0 and array[i] > key:
            array[i + 1] = array[i]
            array[i] = key
            intermediate_variables.append([deepcopy(i), deepcopy(i+1)])
            intermediate_steps.append(list(deepcopy(array)))
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            if arrary[j] < arrary[min_index]:
                ...
                swaps += 1
                intermediate_variables.append([deepcopy(i), deepcopy(j)])
...
def quick_sort(array):
    ...
    def partition(array, left, right):
        ...
        if i != right: 
            intermediate_variables.append([deepcopy(i), deepcopy(right)])
            intermediate_steps.append(list(deepcopy(array)))
        return i 
    ...
    return sort(array, 0, length - 1), intermediate_steps, intermediate_variables, None, None
...
def main(args):
    ...
    for _ in range(data_size):
        ...
        output_arr, intermediate_steps, intermediate_variables, comparisons, swaps = \
            sorting_algorithms[args.save_dir](deepcopy(arr))
        
        # save intermediate steps
        instance = {
            "input": " ".join([str(int(i)) for i in arr]),
        }
        instance.update({
            "output": " ".join([str(int(i)) for i in output_arr]),
        })
        ...
        for k, record in enumerate(records):
            ...
            instance.update({
                f"step_{k}_index_i": intermediate_variables[k][0],
                f"step_{k}_index_j": intermediate_variables[k][1],
            })
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default="insertion_sort")
