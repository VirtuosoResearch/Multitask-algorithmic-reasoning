...
class SinglePromptMultitaskTrainer:
    """
    ...
    def __init__(self, model, tokenizer, optimizer, lr_scheduler,
        ...
        self.test_data_loader = test_data_loader if test_data_loader is not None else valid_data_loader
        self.do_validation = self.valid_data_loader is not None
        self.len_epoch = len(self.train_data_loader)

        self.train_metrics = MetricTracker('loss')
        self.valid_metrics = {f"column_{task}": MetricTracker('loss', 'accuracy', 'edit_distance') for task in output_columns}
    ...
    def _valid_epoch(self, epoch):
        ...
        for step, batch in enumerate(self.valid_data_loader):
            ...
            for column in self.output_columns:
                ...
                self.valid_metrics[task_name].update('accuracy', metrics[f"{task_name}_accuracy"], n=metrics[f"{task_name}_num_samples"])
                self.valid_metrics[task_name].update('edit_distance', metrics[f"{task_name}_edit_distance"], n=metrics[f"{task_name}_num_samples"])

                self.valid_metrics['average'].update('loss', outputs.loss.item(), n=len(batch["labels"]))
                self.valid_metrics['average'].update('accuracy', metrics[f"{task_name}_accuracy"], n=metrics[f"{task_name}_num_samples"])
                self.valid_metrics['average'].update('edit_distance', metrics[f"{task_name}_edit_distance"], n=metrics[f"{task_name}_num_samples"])
