...
def main(config, args):
    ...
    if args.model_interpolate:
        ...
        for key in list(model_end.keys()):
            if "task_head_dict" in key:
                task_heads.append(key.split(".")[1])
        model.add_heads(task_heads)
        model.load_state_dict(model_start, strict=False)
        model.load_state_dict(model_end, strict=False)
        model.load_state_dict(model_inter, strict=False)
    ...
    if (not split_attention) and (args.train_constraint or args.train_constraint_allocation):
        for name, module in model.named_modules():
            if "c_attn" in name:
                split_gpt_self_attention(module, "weight")
    ...
    for run in range(args.runs):
        ...
        checkpoint_dir = os.path.join("saved", 
            ...
            + ("_constraint" if args.train_constraint else "") \
            + ("_ls" if args.train_ls else "")\
            + (f"_constraint_allocation_{args.allocation_strategy}_{args.allocation_alpha}_{args.reg_total}" if args.train_constraint_allocation else "")\
            + ("_sam" if args.train_sam else "")\
            + ("_mixout" if args.train_mixout else "")
            + ("_single_prompt" if args.concatenate_steps else "")
        )
        if args.train_constraint:
            trainer = ConstraintMultitaskTrainer(model, tokenizer, optimizer, lr_scheduler,
                            config=config,
                            device=device,
                            logger=logger,
            ...
            trainer = ConstraintAllocationMultitaskTrainer(model, tokenizer, optimizer, lr_scheduler,
                            config=config,
                            device=device,
                            logger=logger,
                            multitask_train_data_loader=multitask_train_dataloader,
            ...
            trainer.add_constraint(
                    ...
                    alpha=args.allocation_alpha,
                    use_topk=args.use_topk
                )
        elif args.train_ls:
            trainer = LabelSmoothMultitaskTrainer(model, tokenizer, optimizer, lr_scheduler,
                            config=config,
                            device=device,
                            logger=logger,
                            multitask_train_data_loader=multitask_train_dataloader,
            ...
            trainer = SAMMultitaskTrainer(model, tokenizer, optimizer, lr_scheduler,
                            config=config,
                            device=device,
                            logger=logger,
                            multitask_train_data_loader=multitask_train_dataloader,
...
if __name__ == '__main__':
    ...
    args.add_argument("--reg_method", type=str, default="constraint")
    ...
    args.add_argument("--allocation_alpha", type=float, default=1e-2)
    args.add_argument("--use_topk", default=False, action="store_true")
