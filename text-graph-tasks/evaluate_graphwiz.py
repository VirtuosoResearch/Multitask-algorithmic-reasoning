...
class args:
    batch_size = 2
    is_bf16 = True
    save_dir = "./results/graph_wiz/llama2-7b-dpo-pretrained"

    max_tokens = 1024

    model_path = "GraphWiz/LLaMA2-7B-DPO"
...
for lang in tasks:
    ...
    gsm8k_datas = load_dataset("GraphWiz/GraphInstruct-Test", name=lang, split="test")
    
    for i in tqdm(range(start_index, len(gsm8k_datas), batch_size)):
        cur_gsm8k_batch = gsm8k_datas.select(np.arange(i, i + batch_size))
