...
def calc_metrics(key, preds, batch, type_, loc = "node"):
    truth = batch[key]
    preds = preds[key]
    # count ones
    ...
    if loc == "edge":
        if type_ == "pointer":
            """Node Acc. and Graph Acc. This is the same as the Node Pointer"""
            ...
            for n in range(batch.num_graphs):
                gpred = (preds[batch.batch[batch.edge_index[0]] == n]>0.5).bool().cpu().numpy()
                gtruth = truth[batch.batch[batch.edge_index[0]] == n].cpu().numpy()
        ...
        raise NotImplementedError(f"Unknown metric location {loc}")